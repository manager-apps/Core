...
) : IServerClient
{
  ...
  public async Task<IEnumerable<TResponse>> Get<TResponse>(string url, RequestMetadata metadata, CancellationToken cancellationToken)
  {
    ...
    var result = await response.Content.ReadFromJsonAsync<IEnumerable<TResponse>>(cancellationToken: cancellationToken);

    logger.LogInformation("GET request to {Url} completed successfully with status {StatusCode}", url, response.StatusCode);
